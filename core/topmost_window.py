...
def set_foreground_window_by_title(title):
    ...
    if hwnd:
        ...
        try:
            ...
            print_to_chat(f"Error setting foreground window: {e}")
    else:
        print_to_chat(f"Window with title '{title}' not found.")
...
def focus_topmost_window():
    ...
    if topmost_window_title:
        print_to_chat(f"Selected application: {topmost_window_title}")
        set_foreground_window_by_title(topmost_window_title)
        return topmost_window_title
    else:
        print_to_chat("No suitable windows found.")
